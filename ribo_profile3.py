...
class RiboProfile:
    ...
    def termination_scores(self, gene_list = None, add = .01, reads_per_mil = False,
        ...
        '''
        df = self.value_counts.reindex(list(range(1,self.genome_size+1)),fill_value=0)
        term_scores = {}
        flags = {}
        if not gene_list:
        ...
        for protein in gene_list:
            hits_df, start_codon, stop_codon = self.gene_hits_start_zero(
                                                        protein, start_offset=0, stop_offset=0, 
                                                        reads_per_mil=reads_per_mil, df=df
                                                        )

            gene_length = stop_codon - start_codon 
            density = hits_df[hits_df.columns[0]].sum()/hits_df.shape[0]
            if density >= density_filt:
                if gene_length >= gene_length_filt:
    ...
    def gene_hits(self, gene_name, start_offset=50, stop_offset=50, normalize = False, reads_per_mil = False, df=None):
        """*This does not generate 0 values for genomic positions without reads"""
    ...
    def gene_hits_start_zero(self, gene, df=None, start_offset=50, 
        ...
        df2, start_codon, stop_codon = self.gene_hits(gene, 
                                                 df = df,
...
# =============================================================================
    ...
    def make_full_seq_df_term_scores(self, path_to_proteome,
                                    path_fasta_genome,gene_list=None, density_filt=0.25, 
        ...
        if self.feature_type !='CDS':
            raise Exception(f'feature type for ribo_profile3 object is {self.feature_type}\n'
                            'it must be set to "CDS"')
        if density_display:
            term_scores_df = self.termination_scores(gene_list=gene_list, density_filt=density_filt,
                                                        df_return=True, reads_per_mil=reads_per_mil)
            term_scores_df.index.rename('protein ID', inplace=True)
            
            
        else:
            term_scores = self.termination_scores(gene_list=gene_list,density_filt=density_filt)
        ...
        df_DNA_seqs = gu_object.get_DNA_seq_for_proteins(path_fasta_genome,gene_list)
        df_AA_seqs = gu_object.get_get_AA_seq_for_proteins(gene_list)
